...
// TestFetchEndpoints validates Warden response parsing.
...
// TestNormalizeWardenInput_HostOnly verifies host-only input normalization.
func TestNormalizeWardenInput_HostOnly(t *testing.T) {
	scheme, host, err := NormalizeWardenInput("warden.stg.s.bozon.tech")
	if err != nil {
		t.Fatalf("NormalizeWardenInput error: %v", err)
	}
	if scheme != "http" || host != "warden.stg.s.bozon.tech" {
		t.Fatalf("NormalizeWardenInput got %q %q", scheme, host)
	}
}

// TestNormalizeWardenInput_FullURL verifies URL with path normalization.
func TestNormalizeWardenInput_FullURL(t *testing.T) {
	scheme, host, err := NormalizeWardenInput("https://warden.stg.s.bozon.tech/endpoints?service=chatapi-ng.pg:bouncer")
	if err != nil {
		t.Fatalf("NormalizeWardenInput error: %v", err)
	}
	if scheme != "https" || host != "warden.stg.s.bozon.tech" {
		t.Fatalf("NormalizeWardenInput got %q %q", scheme, host)
	}
}
...
func TestNormalizeEndpointMask_AbsoluteURL(t *testing.T) {
	mask, err := NormalizeEndpointMask("http://warden.stg.s.bozon.tech/endpoints?service=%s.pg:bouncer")
	if err != nil {
...
func TestNormalizeEndpointMask_MissingPlaceholder(t *testing.T) {
	if _, err := NormalizeEndpointMask("/endpoints?service=chatapi-ng.pg:bouncer"); err == nil {
...
func TestBuildEndpointURL(t *testing.T) {
	url, err := BuildEndpointURL(
		"http",
		"warden.stg.s.bozon.tech",
		"/endpoints?service=<нужная базка>.pg:bouncer",
		"chatapi-ng",
	)
	if err != nil {
		t.Fatalf("BuildEndpointURL error: %v", err)
	}
	want := "http://warden.stg.s.bozon.tech/endpoints?service=chatapi-ng.pg:bouncer"
	if url != want {
...
// TestExtractEndpointMaskFromWardenInput_FullURL verifies mask extraction from full URL with service.
func TestExtractEndpointMaskFromWardenInput_FullURL(t *testing.T) {
	mask, ok, err := ExtractEndpointMaskFromWardenInput("http://warden.stg.s.bozon.tech/endpoints?service=chatapi-ng.pg:bouncer")
	if err != nil {
		t.Fatalf("ExtractEndpointMaskFromWardenInput error: %v", err)
	}
	if !ok {
		t.Fatalf("ExtractEndpointMaskFromWardenInput expected ok=true")
	}
	if mask != "/endpoints?service=<db>.pg:bouncer" {
		t.Fatalf("ExtractEndpointMaskFromWardenInput mask=%q", mask)
	}
}

// TestExtractEndpointMaskFromWardenInput_HostOnly verifies no mask extraction for host-only input.
func TestExtractEndpointMaskFromWardenInput_HostOnly(t *testing.T) {
	mask, ok, err := ExtractEndpointMaskFromWardenInput("warden.stg.s.bozon.tech")
	if err != nil {
		t.Fatalf("ExtractEndpointMaskFromWardenInput error: %v", err)
	}
	if ok || mask != "" {
		t.Fatalf("ExtractEndpointMaskFromWardenInput got ok=%v mask=%q, want ok=false mask=\"\"", ok, mask)
	}
}

// TestExtractEndpointMaskFromWardenInput_Placeholder verifies preserving explicit placeholder.
func TestExtractEndpointMaskFromWardenInput_Placeholder(t *testing.T) {
	mask, ok, err := ExtractEndpointMaskFromWardenInput("http://warden.stg.s.bozon.tech/endpoints?service=%s.pg:bouncer")
	if err != nil {
		t.Fatalf("ExtractEndpointMaskFromWardenInput error: %v", err)
	}
	if !ok {

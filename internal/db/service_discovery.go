...
// Endpoint describes a Service discovery endpoint entry.
...
// FetchEndpoints gets endpoint data from Service discovery.
func FetchEndpoints(ServiceDiscoveryURL string) ([]Endpoint, error) {
	client := &http.Client{Timeout: 10 * time.Second}
	resp, err := client.Get(ServiceDiscoveryURL)
	if err != nil {
		return nil, fmt.Errorf("fetch Service discovery endpoints: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("read Service discovery response: %w", err)
	}
	...
	if len(endpoints) == 0 {
		return nil, fmt.Errorf("Service discovery returned no endpoints")
	}
...
// NormalizeServiceDiscoveryInput converts an arbitrary Service discovery input to scheme and host.
func NormalizeServiceDiscoveryInput(raw string) (string, string, error) {
	val := strings.TrimSpace(raw)
	if val == "" {
		return "", "", fmt.Errorf("Service discovery URL must not be empty")
	}
	...
	if err != nil {
		return "", "", fmt.Errorf("invalid Service discovery URL: %w", err)
	}
	...
	if scheme != "http" && scheme != "https" {
		return "", "", fmt.Errorf("Service discovery scheme must be http or https")
	}

	host := strings.ToLower(strings.TrimSpace(u.Host))
	if host == "" {
		return "", "", fmt.Errorf("Service discovery host must not be empty")
	}
...
// BuildEndpointURL builds final Service discovery endpoint URL from persisted host + mask + service.
...
// ExtractEndpointMaskFromServiceDiscoveryInput tries to derive endpoint mask from a full Service discovery URL.
func ExtractEndpointMaskFromServiceDiscoveryInput(raw string) (string, bool, error) {
	val := strings.TrimSpace(raw)
	if val == "" {
		return "", false, fmt.Errorf("Service discovery URL must not be empty")
	}
	...
	u, err := url.Parse(candidate)
	if err != nil {
		return "", false, fmt.Errorf("invalid Service discovery URL: %w", err)
	}
